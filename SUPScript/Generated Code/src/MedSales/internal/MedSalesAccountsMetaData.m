...
@implementation MedSalesAccountsMetaData

+ (MedSalesAccountsMetaData*)getInstance
{
    return [[[MedSalesAccountsMetaData alloc] init] autorelease];
}

- (id)init
{
    if (self = [super init]) {
		self.id = _ENTITY_NUMBER_FOR_MedSalesAccounts_;
		self.name = @"Accounts";
		self.klass = [MedSalesAccounts class];
 		self.allowPending = YES;;

		self.isClientOnly = NO;

		SUPObjectList *attributes = [SUPObjectList getInstance];
		SUPAttributeMetaDataRBS* a_account_id = [SUPAttributeMetaDataRBS attributeMetaDataWith:
			322:
			[SUPDataType forName:@"int"]:@"integer":@"account_id":@"":@"a":
			@"":-1:0:0:
			@"null":NO:@"":
			NO:NO:NO:NO:NO:NO:
			GeneratedScheme_NONE:
			NO:SUPPersonalizationType_None:NO];
		[a_account_id setColumn:@"a"];
		SUPAttributeMetaDataRBS* a_account_username = [SUPAttributeMetaDataRBS attributeMetaDataWith:
			323:
			[SUPDataType forName:@"string"]:@"varchar(1200)":@"account_username":@"":@"b":
			@"":1200:0:0:
			@"null":NO:@"":
			NO:NO:NO:NO:NO:NO:
			GeneratedScheme_NONE:
			NO:SUPPersonalizationType_None:NO];
		[a_account_username setColumn:@"b"];
		SUPAttributeMetaDataRBS* a_account_password = [SUPAttributeMetaDataRBS attributeMetaDataWith:
			324:
			[SUPDataType forName:@"string"]:@"varchar(1200)":@"account_password":@"":@"c":
			@"":1200:0:0:
			@"null":NO:@"":
			NO:NO:NO:NO:NO:NO:
			GeneratedScheme_NONE:
			NO:SUPPersonalizationType_None:NO];
		[a_account_password setColumn:@"c"];
		...
		[attributes addThis:a_account_id];
		[attributes addThis:a_account_username];
		[attributes addThis:a_account_password];
		...
		SUPAttributeMap * attributeMap = [SUPAttributeMap getInstance];
 		...
 		SUPOperationMetaData* o_findAll_0 = [SUPOperationMetaData createOperationMetaData:1:(SUPString)@"findAll":[SUPDataType forName:@"Accounts*"]:true];
		[o_findAll_0 setIsStatic:YES];
		[o_findAll_0 setIsCreate:NO];
		[o_findAll_0 setIsUpdate:NO];
		[o_findAll_0 setIsDelete:NO]; 		
 		SUPOperationMetaData* o_findByPrimaryKey_1 = [SUPOperationMetaData createOperationMetaData:2:(SUPString)@"findByPrimaryKey":[SUPDataType forName:@"Accounts"]:true];
	  	{
			SUPObjectList *parameters_list = nil;
	 		SUPParameterMetaData* p_findByPrimaryKey_account_id = [SUPParameterMetaData createParameterMetaData:1:(SUPString)@"account_id":[SUPDataType forName:@"int"]];
 			parameters_list = [SUPObjectList listWithCapacity:1];
			[parameters_list addThis:p_findByPrimaryKey_account_id];
		...
		[o_save_0 setIsDelete:NO]; 		
 		SUPOperationMetaData* o_findWithQuery_1 = [SUPOperationMetaData createOperationMetaData:10:(SUPString)@"findWithQuery":[SUPDataType forName:@"Accounts*"]:true];
	  	{
			SUPObjectList *parameters_list = nil;
		...
		self.table = @"medsales_1_0_accounts";
		...
		[i_findByPrimaryKeyIndex.attributes add:a_account_id];
