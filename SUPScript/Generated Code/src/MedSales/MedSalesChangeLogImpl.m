...
@implementation MedSalesChangeLogImpl
...
static SUPEntityDelegate *g_MedSalesChangeLogImpl_delegate = nil;

+ (SUPEntityDelegate *) delegate
{
	@synchronized(self) {
		if (g_MedSalesChangeLogImpl_delegate == nil) {
			g_MedSalesChangeLogImpl_delegate = [[SUPEntityDelegate alloc] initWithName:@"MedSalesChangeLogImpl" clazz:[self class]
				metaData:[self metaData] dbDelegate:[MedSalesMedSalesDB delegate] database:[MedSalesMedSalesDB instance]];
		}
	}
	
	return [[g_MedSalesChangeLogImpl_delegate retain] autorelease];
}

static SUPEntityMetaDataRBS* MedSalesChangeLogImpl_META_DATA;

+ (SUPEntityMetaDataRBS*)metaData
{
    if (MedSalesChangeLogImpl_META_DATA == nil) {
		MedSalesChangeLogImpl_META_DATA = [[MedSalesChangeLogImplMetaData alloc] init];
	}
	
	return MedSalesChangeLogImpl_META_DATA;
}
...
- (id) init
{
    if ((self = [super init]))
    {
        self.classMetaData = [MedSalesChangeLogImpl metaData];
        [self setEntityDelegate:(SUPEntityDelegate*)[MedSalesChangeLogImpl delegate]];
    }
    return self;    
}
...
+ (MedSalesChangeLogImpl*)find:(MedSalesChangeLogKey*)id_
{
    ...
    return (MedSalesChangeLogImpl*)[(SUPEntityDelegate*)([[self class] delegate]) findEntityWithKeys:keys];
}

+ (SUPObjectList*)findWithQuery:(SUPQuery*)query
{
    return (SUPObjectList*)[(SUPEntityDelegate*)([[self class] delegate])  findWithQuery:query:[MedSalesChangeLogImpl class]];
}

- (MedSalesChangeLogKey*)_pk
{
    return (MedSalesChangeLogKey*)[self i_pk];
}

+ (MedSalesChangeLogImpl*)load:(MedSalesChangeLogKey*)id_
{
    return (MedSalesChangeLogImpl*)[(SUPEntityDelegate*)([[self class] delegate]) load:id_];
}

+ (MedSalesChangeLogImpl*)getInstance
{
    MedSalesChangeLogImpl* me = [[MedSalesChangeLogImpl alloc] init];
    [me autorelease];
    return me;
}
...
{
   return [MedSalesLogRecordImpl findByEntity:@"ChangeLogImpl":[self keyToString]];
}
...
{
    NSString *sql = @"truncate table medsales_1_0_changelogimpl";
    ...
    @try
    {
        [[MedSalesMedSalesDB dblock] writeLock];
        connection = [MedSalesMedSalesDB getConnectionWrapper];
        transaction = ([connection inTransaction] ? nil : [MedSalesMedSalesDB beginTransaction]);
    ...
    @finally {
        ...
        [[MedSalesMedSalesDB dblock] unlock];
    }    
