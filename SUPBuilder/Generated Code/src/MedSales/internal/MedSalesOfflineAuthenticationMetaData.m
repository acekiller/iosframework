...
@implementation MedSalesOfflineAuthenticationMetaData

+ (MedSalesOfflineAuthenticationMetaData*)getInstance
{
    return [[[MedSalesOfflineAuthenticationMetaData alloc] init] autorelease];
}

- (id)init
{
    if (self = [super init]) {
		self.id = _ENTITY_NUMBER_FOR_MedSalesOfflineAuthentication_;
		self.name = @"OfflineAuthentication";
		self.klass = [MedSalesOfflineAuthentication class];
		...
		self.table = @"co_medsales_1_0_offlineauthentication";
